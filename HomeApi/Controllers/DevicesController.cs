...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteById([FromRoute] Guid id)
        {
            var device = await _devices.GetDeviceById(id);
            if (device is null)
                return StatusCode(404, $"Could not find device with id {id}");

            await _devices.DeleteDevice(device);
            return StatusCode(200, $"Device with id {id} successfully deleted");
        }

        [HttpDelete]
        [Route("delete-by-name-{name}")]
        public async Task<IActionResult> DeleteByName([FromRoute] string name)
        {
            var device = await _devices.GetDeviceByName(name);
            if (device is null)
                return StatusCode(404, $"Could not find device {name}");
               
            await _devices.DeleteDevice(device);
            return StatusCode(200, $"Successfully deleted device {name}");
        }
    }
}